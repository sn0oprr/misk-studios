...
export default function StudioCard({ studio, onReserve }: StudioCardProps) {
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
      ...
      <div className="p-6">
        <h3 className="text-xl font-bold text-gray-900 mb-2 font-sf">{studio.name}</h3>
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-1">
            {studio.equipment.slice(0, 3).map((equipement, index) => (
              <span
                key={index}
            ...
            {studio.equipment.length > 3 && (
              <span className="text-xs text-gray-500">
                +{studio.equipment.length - 3} autres
              </span>
            )}
          </div>
        </div>
